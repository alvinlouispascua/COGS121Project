/*
 * The code for the Shelter Page. It lists several homeless shelters in the San Diego area as well their hours 
...
//fills rows with shelter data retrieved from firebase
  database.ref('shelters/1').once('value', (snapshot) => {
      rows[0] = snapshot.val();
    });
    database.ref('shelters/2').once('value', (snapshot) => {
      rows[1] = snapshot.val(); 
    });
      database.ref('shelters/3').once('value', (snapshot) => {
      rows[2] = snapshot.val();
    });
        database.ref('shelters/4').once('value', (snapshot) => {
      rows[3] = snapshot.val();
    });
