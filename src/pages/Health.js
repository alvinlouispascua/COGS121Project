/*
 * The code for the Health Page. It lists several homeless friendly health clinics in the San Diego area as well their hours 
...
//fills rows with health data retrieved from firebase
  database.ref('health/1').once('value', (snapshot) => {
      rows[0] = snapshot.val();
    });
    database.ref('health/2').once('value', (snapshot) => {
      rows[1] = snapshot.val(); 
    });
      database.ref('health/3').once('value', (snapshot) => {
      rows[2] = snapshot.val();
    });
        database.ref('health/4').once('value', (snapshot) => {
      rows[3] = snapshot.val();
    });
